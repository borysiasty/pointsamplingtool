...
class Dialog(QDialog, Ui_Dialog):
    ...
    def __init__(self, iface):
        ...
        for i in range(mapCanvas.layerCount()):
            ...
            elif (layer.type() == layer.VectorLayer) and (
    ...
    def updateFieldsList(self):
        ...
        for j in range(1, len(self.sampItems[i])):
            ...
            self.inData.addItem(
                self.sampItems[i][0].name()
                + " : "
                + self.sampItems[i][j][0]
                + " (source point)"
            )
    ...
    def fieldNameChanged(
        self, n
    ):  # called when any cell of the fieldsTable was modyfied
    ...
    def outFile(self):  # by Carson Farmer 2008
        ...
        if (
            not outPath.upper().endswith(".GPKG")
            and not outPath.upper().endswith(".CSV")
            and not outPath.upper().endswith(".SHP")
        ):
            outPath += ".gpkg"
        if outName:
            self.outShape.clear()
    ...
    def accept(
        ...
        if self.inSample.currentText() == "":
            ...
            QMessageBox.information(
                self,
                self.tr("Point Sampling Tool"),
                self.tr("Please select vector layer containing the sampling points"),
            )
            return
        if nothingSelected:
            self.tabWidget.setCurrentIndex(0)
            QMessageBox.information(
                self,
                self.tr("Point Sampling Tool"),
                self.tr("Please select at least one polygon attribute or raster band"),
            )
            return
        if self.outShape.text() == "":
            ...
            QMessageBox.information(
                self,
                self.tr("Point Sampling Tool"),
                self.tr("Please specify output file name"),
            )
            return
                ...
                if not outPath.upper().endswith(".GPKG"):
                    if (
                        ...
                        self.statusLabel.setText(
                            self.tr("Fill up the input fields, please.")
                        )
                        self.repaint()
                        return
                else:
                    msg = (
                        self.tr("""Please provide <b>table name</b> for your layer.<br/>
                      <b>WARNING: </b>Database %s already exists. If you select a table existing in it, the table will be overwritten.""")
                        % outName
                    )
                ...
                if self.vlayer.isValid():
                    ...
                    for layer in QgsProject.instance().mapLayers().values():
                        if (
                            ...
                            and layer.source() == self.vlayer.source()
                        ):
                            QgsProject.instance().removeMapLayer(layer)
                    QgsProject.instance().addMapLayer(self.vlayer)
                    self.statusLabel.setText(
                        self.tr("OK. The new layer has been added to the map.")
                    )
                else:
    ...
    def sampling(self, outPath, tableName):  # main process
        ...
        memLayer = QgsVectorLayer(
            "Point?crs=epsg:%d" % sRs.postgisSrid(), "temp layer", "memory"
        )
        ...
        for pointFeat in pointProvider.getFeatures():
            ...
            if (
                snp < 100
                or (snp < 5000 and (np // 10.0 == np / 10.0))
                or (np // 100.0 == np / 100.0)
            ):  # display each or every 10th or every 100th point:
                self.statusLabel.setText(
                    self.tr("Processing point %s of %s") % (np, snp)
                )
                self.repaint()
            ...
            bBox = QgsRectangle(
                ...
                pointPoint.x() + 0.001,
                pointPoint.y() + 0.001,
            )  # reuseable rectangle buffer around the point feature
            previousPolyLayer = (
                None  # reuse previous feature if it's still the same layer
            )
            previousPolyFeat = (
                None  # reuse previous feature if it's still the same layer
            )
            ...
            for i in range(len(self.fields)):
                ...
                if field[0] == "point":
                    ...
                    if rastLayer == previousRastLayer:
                        ...
                        rastSample = (
                            rastLayer.dataProvider()
                            .identify(pointPoint, QgsRaster.IdentifyFormatValue)
                            .results()
                        )
                    try:
